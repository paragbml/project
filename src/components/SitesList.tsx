  ...
  React.useEffect(() => {
    const filtered = sites.filter(site => {
      const matchesReligion = selectedReligion === 'all' || 
        site.religion.toLowerCase() === selectedReligion.toLowerCase();
        ...
        {filteredSites.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredSites.map(site => (
              <div key={site.id} className="h-full transform transition-transform duration-300 hover:-translate-y-2">
                <SiteCard site={site} />
              </div>
            ))}
          </div>
